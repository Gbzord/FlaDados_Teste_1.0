---
title: "FlaDados - Analytics Tático CR Flamengo"
output: 
...
analise_brasileirao_ofensivo <- tribble(
  ...
  mutate(
    ...
    # Ajustei a métrica, pois a condução progressiva é uma métrica de volume e deve ser somada ao volume esperado (xG/jogos)
...
# Criando o dataframe defensivo
analise_brasileirao_defensivo <- tribble(
  ~time, ~jogos, ~Press, ~PPDA, # <- Coluna 'jogos' em minúsculo
  ...
  mutate(
  eficiencia_defensiva = Press / PPDA, 
  )
...
# =========================================================================
# DADOS TÁTICOS (Normalização)
# =========================================================================

# 3.2. Simulação de Eventos (Para o Mapa Tático)
analise_tatico <- analise_brasileirao_ofensivo %>%
  # Normalização das métricas para mapear no campo
  mutate(
    # Mapeamento: Criatividade (y) e Intensidade (x) para coordenadas táticas
...
dados_campo_uefa <- analise_tatico 
analise_brasileirao <- analise_brasileirao_ofensivo

# CONFIGURAÇÃO DE CORES DO FLAMENGO
cores_flamengo <- c("#C12C25", "#000000", "#FFFFFF")

# =========================================================================
# LÓGICA REATIVA (SHINY) - FILTRO DE DADOS
# =========================================================================

filtered_data <- eventReactive(input$atualizar, {
  # Corrigido: Usando 'jogos' (minúsculo) em vez de 'Jogos'
...
```{r}
selectInput("time_selecionado", "Time para Análise:",
            choices = unique(analise_brasileirao_ofensivo$time),
            selected = "Flamengo")

sliderInput("jogos_minimos", "Mínimo de Jogos:",
...
renderPlotly({
  ...
  p1 <- ggplot(data_filtered, 
    ...
    labs(title = "Eficiência de Finalização - Gols vs xG",
...
renderUI({
  ...
  tagList(
    ...
    valueBox(value = round(Eficiencia, 2), caption = "Eficiência (Gols/xG)", icon = icon("chart-line"), 
...
```{r}
# GRÁFICO DE DISPERSÃO (Prioridade Alta)
renderPlotly({
  data_filtered <- filtered_data()$ofensiva
  ...
  p2 <- ggplot(data_filtered, 
    ...
    labs(title = "Criatividade vs Intensidade Ofensiva",
         x = "Criatividade Ofensiva (Ast + Prog Passes / Jogo)", y = "Intensidade de Ataque (Ações / Jogo)") +
    theme_minimal() +
    theme(legend.position = "none")
...
### Pressing e Eficiência Defensiva
...
renderPlotly({
  ...
  p3 <- ggplot(data_filtered, 
    ...
    geom_text_repel(aes(label = time), size = 3, max.overlaps = 15) +
    scale_color_manual(values = c("Flamengo" = cores_flamengo[1], "Outros" = "gray")) +
    labs(title = "Análise de Pressing",
         subtitle = "PPDA mais baixo = defesa mais eficiente",
         x = "Intensidade de Pressing (Ações)", y = "PPDA") +
    theme_minimal() +
    theme(legend.position = "none")
...
renderUI({
  ...
  <div style='background: #f8f9fa; padding: 15px; border-radius: 5px; height: 100%;'>
    <h4>🔍 Análise Defensiva</h4>
    <p><strong>PPDA (Passes por Ação Defensiva):</strong> Quanto menor, mais intenso é o pressing no campo adversário. Um valor abaixo de 10 é considerado de elite.</p>
    <p><strong>Eixo Y (PPDA):</strong> O objetivo é estar na parte inferior (baixo PPDA).</p>
    <p><strong>Tamanho do Ponto:</strong> Representa a Eficiência Defensiva calculada (ex: xG sofrido por jogo), indicando a solidez da defesa.</p>
    <p style='color:#C12C25; font-weight: bold;'>O Flamengo deve buscar um PPDA baixo e alta eficiência (ponto grande).</p>
  </div>
  ")
})
...
renderPlot({
  ...
  ggplot(dados_interativo, aes(x = x_coordenada_tatico, y = y_coordenada_tatico)) +
    ...
    scale_color_manual(values = c("Destaque" = cores_flamengo[1], 
                                  "Normal" = "yellow")) +
    theme_pitch() +
    labs(title = "Posicionamento Tático no Espaço de Métrica",
         subtitle = "Eixos baseados na média da liga",
         x = "Criatividade (Mais à direita = Mais Criativo)",
...
### Análise Personalizada (Relatório Flamengo)
...
renderUI({
  ...
  if (nrow(dados_flamengo_of) == 0 | nrow(dados_flamengo_def) == 0) {
    return(HTML("<p>Dados do time selecionado não disponíveis (Verifique o filtro de Jogos Mínimos).</p>"))
  }
  ...
  HTML(paste0(
    "<div style='background: linear-gradient(135deg, ", cores_flamengo[1], ", ", cores_flamengo[2], "); 
      ...
      <h2>📈 Relatório ", input$time_selecionado ,"</h2>
      <div style='display: grid; grid-template-columns: 1fr; gap: 15px; margin-top: 20px;'>
        <div style='background: rgba(255,255,255,0.1); padding: 10px; border-radius: 5px;'>
          <h4>⚽ OFENSIVA</h4>
          <p><strong>Gols:</strong> ", dados_flamengo_of$Gols_Gls[1], " (xG: ", round(dados_flamengo_of$x_G[1], 1), ")</p>
          <p><strong>Eficiência:</strong> ", round(dados_flamengo_of$eficiencia_finalizacao[1], 2), 
        ...
        <div style='background: rgba(255,255,255,0.1); padding: 10px; border-radius: 5px;'>
          ...
          <p><strong>Eficiência Defensiva:</strong> ", round(dados_flamengo_def$eficiencia_defensiva[1], 1), "</p>
        </div>
      </div>
    </div>"
  ))
